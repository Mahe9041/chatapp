...
const NewConversationModal = ({ onClose, }) => {
    ...
    return (_jsx("div", { className: styles.modalOverlay, onClick: onClose, children: _jsxs("div", { className: styles.modal, onClick: (e) => e.stopPropagation(), children: [_jsxs("div", { className: styles.modalHeader, children: [_jsx("h2", { className: styles.modalTitle, children: "New conversation" }), _jsx("button", { className: styles.modalClose, onClick: onClose, children: "\u2715" })] }), _jsxs("div", { className: styles.modalTabs, children: [_jsx("button", { className: `${styles.modalTab} ${tab === "dm" ? styles.modalTabActive : ""}`, onClick: () => {
                                setTab("dm");
                                setSelected([]);
                            }, children: "Direct message" }), _jsx("button", { className: `${styles.modalTab} ${tab === "group" ? styles.modalTabActive : ""}`, onClick: () => {
                                setTab("group");
                                setSelected([]);
                            }, children: "Group chat" })] }), tab === "group" && (_jsx("input", { className: styles.modalInput, placeholder: "Group name", value: groupName, onChange: (e) => setGroupName(e.target.value) })), selected.length > 0 && (_jsx("div", { className: styles.selectedPills, children: selected.map((u) => (_jsxs("span", { className: styles.pill, children: [u.displayName, _jsx("button", { onClick: () => toggleSelect(u), children: "\u2715" })] }, u.id))) })), _jsx("input", { className: styles.modalInput, placeholder: "Search by name or email...", value: query, onChange: (e) => setQuery(e.target.value), autoFocus: true }), _jsxs("div", { className: styles.searchResults, children: [isSearching && _jsx("div", { className: styles.searchHint, children: "Searching..." }), !isSearching && query && results.length === 0 && (_jsx("div", { className: styles.searchHint, children: "No users found" })), !isSearching && !query && (_jsx("div", { className: styles.searchHint, children: "Type to search for users" })), results.map((user) => {
...
const MessageBubble = ({ message, isOwn, }) => {
    ...
    const readByOthers = Object.entries(message.deliveryStatus ?? {}).some(([uid, status]) => uid !== message.senderId && status === "read");
    return (_jsx("div", { className: `${styles.bubbleWrap} ${isOwn ? styles.ownWrap : styles.otherWrap}`, children: _jsxs("div", { className: `${styles.bubble} ${isOwn ? styles.ownBubble : styles.otherBubble}`, children: [message.content?.text && (_jsx("p", { className: styles.bubbleText, children: message.content.text })), message.content?.url && message.type === "image" && (_jsx("img", { src: message.content.url, alt: message.content.originalName ?? "image", style: { maxWidth: "100%", borderRadius: 8, display: "block" } })), message.content?.url && message.type === "audio" && (_jsx("audio", { controls: true, src: message.content.url, style: { width: "100%" } })), message.content?.url && message.type === "video" && (_jsx("video", { controls: true, src: message.content.url, style: { maxWidth: "100%", borderRadius: 8 } })), message.content?.url && message.type === "document" && (_jsxs("a", { href: message.content.url, target: "_blank", rel: "noopener noreferrer", style: {
                        fontSize: 13,
                        color: "inherit",
                        textDecoration: "underline",
                    }, children: ["\uD83D\uDCC4 ", message.content.originalName ?? "Download file"] })), _jsxs("div", { className: styles.bubbleMeta, children: [message.editedAt && _jsx("span", { className: styles.editedTag, children: "edited" }), _jsx("span", { className: styles.bubbleTime, children: new Date(message.createdAt).toLocaleTimeString([], {
                                hour: "2-digit",
                                minute: "2-digit",
...
const TypingIndicator = ({ names }) => {
    if (!names.length)
        return null;
    return (_jsxs("div", { className: styles.typingWrap, children: [_jsxs("div", { className: styles.typingDots, children: [_jsx("span", {}), _jsx("span", {}), _jsx("span", {})] }), _jsx("span", { className: styles.typingText, children: names.length === 1
                    ? `${names[0]} is typing`
